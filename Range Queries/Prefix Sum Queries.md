...
void updateTree(int tl, int tr, int treeNode, int idx, int value) {
    if (tl == tr) {
        tre[treeNode] = {value, max((int)0, value)};
        return;
    }
    int tm = (tl + tr) / 2;
...
void solve() {
    ...
    fo(i, q) {
        ...
        if (x == 1) {
            ...
            cin >> u1 >> u2;
            u1--; u2--;
            cout << query(0, n - 1, 1, u1, u2).pre_max << endl;
        }
    }
    return;
}
