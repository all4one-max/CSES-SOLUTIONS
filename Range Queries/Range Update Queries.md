...
void updateTree(int tl, int tr, int treeNode, int l, int r, int add) {
    if (l > r) return;
    if (tl == l && tr == r) {
        tre[treeNode] += add;
        return;
    ...
    int tm = (tl + tr) / 2;
    updateTree(tl, tm, 2 * treeNode, l, min(tm, r), add);
    updateTree(tm + 1, tr, 2 * treeNode + 1, max(tm + 1, l), r, add);
    return;
}
...
int query(int tl, int tr, int treeNode, int pos) {
    ...
    int tm = (tl + tr) / 2;
    if (pos <= tm) return tre[treeNode] + query(tl, tm, 2 * treeNode, pos);
    else return tre[treeNode] + query(tm + 1, tr, 2 * treeNode + 1, pos);
}
